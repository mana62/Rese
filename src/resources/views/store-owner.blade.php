...
@section('css')
    <link rel="stylesheet" href="{{ asset('css/store-owner.css') }}">
@endsection
...
@section('content')
    ...
    <div class="store-management">
        ...
        <div class="store-form">
            <h2>店舗情報を作成</h2>
            <form action="{{ route('store-owner.createStore') }}" method="POST" enctype="multipart/form-data">
                @csrf
        ...
        <div class="store-info">
            <h2>店舗情報の更新</h2>
            @if ($restaurant)
                <form action="{{ route('store-owner.updateStore') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <label for="name">店舗名</label>
                    @error('name')
                        <div class="store-info__error">{{ $message }}</div>
